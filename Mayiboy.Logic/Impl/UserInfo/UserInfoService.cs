...
namespace Mayiboy.Logic.Impl
{
	...
	public class UserInfoService : BaseService, IUserInfoService
	{
		...
		{
			var response = new QueryUserInfoByIdResponse();
			try
			{
				...
				if (entity != null)
				{
					response.Entity = entity.As<UserInfoDto>();
				}
			}
			catch (Exception ex)
			{
				...
				response.MessageCode = "-1";
				response.MessageText = ex.Message;
				LogManager.LogicLogger.ErrorFormat("根据用户Id查询用户基本信息出错：{0}", new { request, err = ex.ToString() }.ToJson());
			}
			return response;
		}
