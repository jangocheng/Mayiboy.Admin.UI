/**
 ...
 @Name：layui.myvalidation 验证input
...
layui.define('layer', function (exports) {
    ...
    //全局设置
    Validation.prototype.set = function (options) {
        var that = this;
    ...
    //规则设定
    Validation.prototype.verify = function (settings) {
        var that = this;
    ...
    //事件监听
    Validation.prototype.on = function (events, callback) {
        return layui.onevent.call(this, MOD_NAME, events, callback);
    };

    //控件渲染
    Validation.prototype.render = function (type, filter) {
        var that = this
    ...
    //提交校验
    var isvalidation = function (validationclass) {
        var button = $(validationclass);
        var verify = validation.config.verify;
        var stop = null;
        var DANGER = 'layui-form-danger';
        var field = {}
        var elem = button.find(ELEM)

        , verifyElem = button.find('*[lay-verify]') //获取需要校验的元素
        , formElem = button.find('form')[0] //获取当前所在的form元素，如果存在的话
        , fieldElem = button.find('input,select,textarea') //获取所有表单域
    ...
    layer.IsValidation = isvalidation;

    exports(MOD_NAME, validation);
});
