(function ($) {
    $.maintab = {
        ...
        scrollToTab: function (element) {
            var marginLeftVal = $.maintab.calSumWidth($(element).prevAll()), marginRightVal = $.maintab.calSumWidth($(element).nextAll());
            var tabOuterWidth = $.maintab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            var visibleWidth = $(".content-tabs").outerWidth(true) - tabOuterWidth;
            var scrollVal = 0;
        ...
        loadMenu: function () {
            ...
            $.each(data, function (i) {
                ...
                if (row.F_ParentId == "0") {
                    ...
                    _html += '</a>';
                    var childNodes = $.mainindex.jsonWhere(data, function (v) { return v.F_ParentId == row.F_ModuleId });
                    if (childNodes.length > 0) {
                        _html += '<ul class="treeview-menu">';
                        $.each(childNodes, function (i) {
                            var subrow = childNodes[i];
                            var subchildNodes = $.mainindex.jsonWhere(data, function (v) { return v.F_ParentId == subrow.F_ModuleId });
                            _html += '<li>';
